...
run_tests() {
  local devices=("$@")
  local pytest_args=${PYTEST_ARGS:-"--tb=short -v"}
  
  for device in "${devices[@]}"; do
    echo "Running tests for $device..."
    # shellcheck disable=SC2086
    python3 -m pytest ${pytest_args} --device "$device" -k "hedera" --golden_run --display
  done
...
build_devices() {
  ...
  for device in "${devices[@]}"; do
    ...
    BOLOS_SDK="$sdk_path" make DEBUG=1 TEST_PUBLIC_KEY=1 BYPASS_TRANSACTION_ID_CHECK=1 -j8 || exit 1
  done
}
