...
- Rust's memory safety guarantees help prevent many BSS-related issues, but the same rules apply.

- **Avoid mutable static variables** (`static mut`) in `CHECK_ADDRESS` and `GET_PRINTABLE_AMOUNT` handlers.

- Use **stack-allocated variables** and function parameters exclusively during these handlers.

- The Rust SDK's types (like `CheckAddressParams` and `PrintableAmountParams`) are designed to work with stack allocation, making it natural to follow the BSS rules.

- Only write to global state during `SIGN_TRANSACTION` phase when copying parameters for later validation.
