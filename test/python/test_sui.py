...
class SuiSwaptTokenTests(ExchangeTestRunner):
    currency_configuration = cal.SUI_USDC_CURRENCY_CONFIGURATION
    valid_destination_1 = SUI.FOREIGN_ADDRESS
    valid_destination_2 = SUI.FOREIGN_ADDRESS_2
    valid_refund = SUI.OWNED_ADDRESS
    valid_send_amount_1 = SUI.USDC_AMOUNT
    valid_send_amount_2 = SUI.USDC_AMOUNT_2
    valid_fees_1 = SUI.FEES
    valid_fees_2 = SUI.FEES_2
    fake_refund = SUI.FOREIGN_ADDRESS
    fake_payout = SUI.FOREIGN_ADDRESS
    ...
    def perform_final_tx(self, destination, send_amount, fees, _memo):
        sui = SuiClient(self.backend, verbose=False)
        [tx, obj_list] = sui.build_simple_transaction_with_object_list(SUI.OWNED_ADDRESS, destination, send_amount, fees)
        signature = sui.sign_transaction(SUI.SUI_PACKED_DERIVATION_PATH_LE, tx, obj_list)
